...
namespace InfoNet {
	...
	// delegate for InfoNet callbacks
	public delegate bool FuncUpdateGUI(InfoNetProfile& Profile, InfoNetDynamic& Var);

	/// <summary>
	/// Summary for MyForm
	/// </summary>
	public ref class InfoNetUI : public System::Windows::Forms::Form
	{
	public:
		InfoNetUI()
		{
			...
			FuncUpdateGUI^ fpUpdateGUI = gcnew FuncUpdateGUI(this, &InfoNetUI::InfoNetUI_UpdateGUI);
			...
			Viewer = new SERVICE_InfoNet();

			// read program settings from disk
			Viewer->clearLogLevel();
			if (false == Viewer->readProfile(getConfigPath(APP_PROGRAM))){
				...
				Viewer->clearLogLevel();
				CmString ConfigPath = getConfigPath(APP_PROGRAM);
				if (ConfigPath.isEmpty()){
					MessageBoxA(NULL, "Config path could not be opened.", "InfoNet", MB_OK);
				}
				Viewer->processMessageGUI("Close", getConfigPath(APP_PROGRAM), 0);
				return;
#endif
			}

			// application version
			Text = "InfoNet " + APP_VERSION + " - Information Processing Network (IPN) ";
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->button_Close->Click += gcnew System::EventHandler(this, &InfoNetUI::button_Close_Click);
			// 
			// listBoxTouchMode
			// 
			...
			this->listBoxTouchMode->SelectedIndexChanged += gcnew System::EventHandler(this, &InfoNetUI::listBoxTouchMode_SelectedIndexChanged);
			// 
			// listBoxRemoteDevice
			// 
			...
			this->listBoxRemoteDevice->SelectedIndexChanged += gcnew System::EventHandler(this, &InfoNetUI::listBoxRemoteDevice_SelectedIndexChanged);
			// 
			// pictureBoxLocal
			// 
			...
			this->listBoxLocalDevice->SelectedIndexChanged += gcnew System::EventHandler(this, &InfoNetUI::listBoxLocalDevice_SelectedIndexChanged);
			// 
			// timerUpdate
			// 
			this->timerUpdate->Enabled = true;
			this->timerUpdate->Interval = 40;
			this->timerUpdate->Tick += gcnew System::EventHandler(this, &InfoNetUI::timerUpdateGUI_Tick);
			// 
			// InfoNetUI
			// 
			...
			this->Name = L"InfoNetUI";
			this->StartPosition = System::Windows::Forms::FormStartPosition::Manual;
			this->Text = L"InfoNet - Information Processing Network (IPN)";
			this->FormClosing += gcnew System::Windows::Forms::FormClosingEventHandler(this, &InfoNetUI::InfoNetUI_FormClosing);
			this->LocationChanged += gcnew System::EventHandler(this, &InfoNetUI::InfoNetUI_LocationChanged);
			this->SizeChanged += gcnew System::EventHandler(this, &InfoNetUI::InfoNetUI_SizeChanged);
...
#pragma endregion
	...
	private: CmString getConfigPath(char* Program){
		...
		if (false == ConfigPath.getConfigPath(AppDataPath, APP_COMPANY, Program, APP_PROGRAM, APP_VERSION)){
			ConfigPath = "";
		}
		return ConfigPath;
	}
	...
	private: System::Void InfoNetUI_LocationChanged(System::Object^  /*sender*/, System::EventArgs^  /*e*/) {
		Viewer->setWindow(CmPoint(Left, Top), CmSize(Width, Height));
	}
	private: System::Void InfoNetUI_SizeChanged(System::Object^  /*sender*/, System::EventArgs^  /*e*/) {
		Viewer->setWindow(CmPoint(Left, Top), CmSize(Width, Height));
	...
	private: System::Void InfoNetUI_FormClosing(System::Object^  /*sender*/, System::Windows::Forms::FormClosingEventArgs^  /*e*/) {
		isShutDown = true;
		if (false == Viewer->processMessageGUI("Close", getConfigPath(APP_PROGRAM), 0)){
			MessageBoxA(NULL, Viewer->getDynamic().Message.getText(), Viewer->getDynamic().Context.getText(), MB_OK);
		}
	}
	private: System::Void button_Save_Click(System::Object^  /*sender*/, System::EventArgs^  /*e*/) {
		if (false == Viewer->processMessageGUI("Save", getConfigPath(APP_PROGRAM), 0)){
			MessageBoxA(NULL, Viewer->getDynamic().Message.getText(), Viewer->getDynamic().Context.getText(), MB_OK);
		}
	}

	// GUI update
	private: bool InfoNetUI_UpdateGUI(InfoNetProfile& _pro, InfoNetDynamic& /*_dyn*/) {
